...
def create_board():
    board = np.zeros((ROW_COUNT, COULUMN_COUNT))
    return board
...
def winning_move(board, piece):
 # Check horizontal locations for win
    for c in range(COULUMN_COUNT-3):
        for r in range(ROW_COUNT):
            if board[r][c] == piece and board[r][c+1] == piece and board[r][c+2] == piece and board[r][c+3] == piece:
                return True

 # Check vertical locations for win
    for c in range(COULUMN_COUNT):
 ...
 # Check negetively sloped diaganols for win
    for c in range(COULUMN_COUNT-3):
...
width = COULUMN_COUNT * SQUARESIZE
